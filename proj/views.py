...
class LatestStatus(APIView):
    ...
    '''
    def get(self,request,log_fid,format=None):
        try:
            slatest = StatusLog.objects.filter(log_fid=log_fid).order_by('-timestamp')[:15]
            if not slatest:
...
class LastStatusLog(APIView):
    ...
    def get(self,request,log_fid,format=None):
        try:
            last = StatusLog.objects.filter(log_fid=log_fid).last()
        except StatusLog.DoesNotExist:
            return Response({},status=200)
